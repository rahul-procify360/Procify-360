...
// WP-Optimize advanced-cache.php (written by version: 3.8.0) (homeurl: http://127.0.0.1/procify360/) (abspath: C:\xampp\htdocs\procify360/) (do not change this line, it is used for correctness checks)
...
$possible_plugin_locations = array(
	...
	'C:\xampp\htdocs\procify360\wp-content\plugins\wp-optimize\cache',
);
...
if (false === $plugin_location) {
	...
	if (strcasecmp('http://127.0.0.1/procify360/', $protocol . '://' . $host . $request_uri) === 0) {
...
if (false !== $plugin_location) {
	...
	if (!defined('WPO_CACHE_EXT_DIR')) define('WPO_CACHE_EXT_DIR', 'C:\xampp\htdocs\procify360\wp-content\plugins\wp-optimize\cache/extensions');
}

if (!@file_exists(WPO_CACHE_CONFIG_DIR . '/config-127.0.0.1.php')) { return; }

$GLOBALS['wpo_cache_config'] = @json_decode(file_get_contents(WPO_CACHE_CONFIG_DIR . '/config-127.0.0.1.php'), true);

if (empty($GLOBALS['wpo_cache_config'])) {
	include_once(WPO_CACHE_CONFIG_DIR . '/config-127.0.0.1.php');
}
