...
add_action('aheto_before_aheto_testimonials_register', 'prego_testimonials_layout2');

/**
 * Testimonials
 */

function prego_testimonials_layout2($shortcode) {

    $preview_dir = '//assets.aheto.co/testimonials/previews/';

	$shortcode->add_layout('prego_layout2', [
		'title' => esc_html__('Prego Testimonials Simple', 'prego'),
		'image' => $preview_dir . 'prego_layout2.jpg',
	]);

	$shortcode->add_dependecy('prego_testimonials', 'template', ['prego_layout2']);
    $shortcode->add_dependecy('prego_items_bg', 'template', 'prego_layout2');
    $shortcode->add_dependecy('prego_name_use_typo', 'template', 'prego_layout2');
    $shortcode->add_dependecy('prego_name_typo', 'template', 'prego_layout2');
    $shortcode->add_dependecy('prego_name_typo', 'prego_name_use_typo', 'true');
    $shortcode->add_dependecy('prego_text_use_typo', 'template', 'prego_layout2');
    $shortcode->add_dependecy('prego_text_typo', 'template', 'prego_layout2');
	...
	$shortcode->add_params([
        ...
        'prego_items_bg'   => [
            'type'      => 'colorpicker',
            'heading'   => esc_html__( 'Items background color', 'prego' ),
            'grid'      => 6,
            'default'   => '#f7f7f7',
            'selectors' => [
                '{{WRAPPER}} .aheto-contents__item' => 'background: {{VALUE}}',
            ],
        ],
