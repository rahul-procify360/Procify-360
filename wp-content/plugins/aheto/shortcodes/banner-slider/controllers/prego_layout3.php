...
add_action('aheto_before_aheto_banner-slider_register', 'prego_banner_slider_layout3');

/**
 *  Banner Slider
 */

function prego_banner_slider_layout3( $shortcode ) {
    $preview_dir = '//assets.aheto.co/banner-slider/previews/';

	$shortcode->add_layout('prego_layout3', [
		'title' => esc_html__('Prego Banner Slider Creative', 'prego'),
		'image' => $preview_dir . 'prego_layout3.jpg',
	]);
	aheto_demos_add_dependency(['use_heading', 't_heading'], ['prego_layout3'], $shortcode);

	$shortcode->add_dependecy('prego_overlay_color', 'overlay', 'true');
	$shortcode->add_dependecy('prego_subtitle_use_typo', 'template', 'prego_layout3');
	$shortcode->add_dependecy('prego_subtitle_typo', 'template', 'prego_layout3');
	$shortcode->add_dependecy('prego_subtitle_typo', 'prego_subtitle_use_typo', 'true');
	$shortcode->add_dependecy('prego_modern_banners', 'template', 'prego_layout3');
	...
	\Aheto\Params::add_carousel_params($shortcode, [
		...
		'dependency'     => ['template', ['prego_layout3']]
	]);
}
