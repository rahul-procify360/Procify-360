...
class PluginUpdateChecker_2_3 {
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			...
			// Ensure $pluginInfo is an object
			if (!is_object($pluginInfo)) {
				trigger_error('$pluginInfo is not an object.', E_USER_WARNING);
				return;
			}
			
			// Ensure $this->pluginFile is not empty
			if (empty($this->pluginFile)) {
				trigger_error('$this->pluginFile is empty.', E_USER_WARNING);
				return;
			}
			
			// Assign filename to $pluginInfo
			$pluginInfo->filename = basename($this->pluginFile); 
